...
public class FilterTabsView extends FrameLayout {
    ...
    public class Tab {
        ...
        public boolean setTitle(String newTitle) {
            newTitle = viksConfig.tabIcons != 2 ? newTitle : "";
            realTitle = newTitle;
    ...
    public class TabView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float textX = ((getMeasuredWidth() - tabWidth) / 2f) + currentTab.iconWidth - (viksConfig.tabIcons == 0 ? AndroidUtilities.dp(2) : 0);
            if (animateTextX) {
            ...
            if (animateCounterEnter || counterText != null || showRemove && (isEditing || editingStartAnimationProgress != 0)) {
                ...
                int textSpace = AndroidUtilities.dp(viksConfig.tabIcons == 2 && viksConfig.tabStyle <= 2 ? 3 : 6);
    ...
    private final Property<FilterTabsView, Float> COLORS = new AnimationProperties.FloatProperty<FilterTabsView>("animationValue") {
        ...
        public void setValue(FilterTabsView object, float value) {
            ...
            selectorDrawable.setColor(ColorUtils.setAlphaComponent(ColorUtils.blendARGB(color1, color2, value), viksConfig.tabStyle >= 3 ? 0x2F : 0xFF));
    ...
    public FilterTabsView(Context context) {
        ...
        listView.setSelectorType(viksConfig.tabStyle >= 3 ? 100 : 8);
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (isEditing) {
                if (position != 0 || viksConfig.hideAllChats) {
                    int side = AndroidUtilities.dp(6);
                    if (tabView.rect.left - side < x && tabView.rect.right + side > x) {
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (child == listView) {
            ...
            if (indicatorWidth != 0 && viksConfig.tabStyle != 2) {
                canvas.save();
                ...
                updateSelector();
                selectorDrawable.setBounds((int) indicatorX - (viksConfig.tabStyle == 3 ? AndroidUtilities.dp(10) : viksConfig.tabStyle == 4 ? AndroidUtilities.dp(12) : 0), viksConfig.tabStyle >= 3 ? height / 2 - AndroidUtilities.dp(viksConfig.tabStyle == 3 ? 17 : 18) : height - AndroidUtilities.dpr(4), (int) (indicatorX + indicatorWidth) + (viksConfig.tabStyle == 3 ? AndroidUtilities.dp(10) : viksConfig.tabStyle == 4 ? AndroidUtilities.dp(12) : 0), viksConfig.tabStyle >= 3 ? height / 2 + AndroidUtilities.dp(viksConfig.tabStyle == 3 ? 17 : 18) : height);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (!tabs.isEmpty()) {
            ...
            int trueTabsWidth;
            if (!viksConfig.hideAllChats && findDefaultTab() != null)  {
    ...
    public void notifyTabCounterChanged(int id) {
        ...
        if (oldWidth != width || invalidated) {
            ...
            adapter.notifyDataSetChanged();
            allTabsWidth = 0;
            if (!viksConfig.hideAllChats && findDefaultTab() != null) findDefaultTab().setTitle(LocaleController.getString("FilterAllChats", R.string.FilterAllChats));
...
//                notifyItemMoved(i, i + 1);
    ...
    public class TouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
            if (!viksConfig.hideAllChats && (!isEditing || (viewHolder.getAdapterPosition() == 0 && tabs.get(0).isDefault && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium()))) {
        ...
        public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder source, RecyclerView.ViewHolder target) {
            if (!viksConfig.hideAllChats && ((source.getAdapterPosition() == 0 || target.getAdapterPosition() == 0) && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium())) {
                return false;
            }
    ...
    public void updateSelector() {
        selectorDrawable.setColor(ColorUtils.setAlphaComponent(Theme.getColor(tabLineColorKey), viksConfig.tabStyle >= 3 ? 0x2F : 0xFF));
